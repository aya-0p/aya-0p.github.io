...
function login(id) {
  ...
  if (uid === null || uid === undefined || uid === "") {
    ...
    document.getElementById("nav11").style.display = "none";
...
function wShare() {
  login();
  if (checkJson(localStorage.getItem("document_list"))　 && JSON.parse(localStorage.getItem("document_list")).length > 1) {
...
function wMove() {
  var worldList = JSON.parse(localStorage.getItem("document_gravel")); //固定
  var list = worldList.splice(1, worldList.length - 1);
...
function worldMove() {
  ...
  if (wName === null || wName === undefined || wName === "" || !checkJson(localStorage.getItem(wName))) {} else {
    var document_gravel = JSON.parse(localStorage.getItem("document_gravel")); //固定
    var document_list = JSON.parse(localStorage.getItem("document_list"));
    document_gravel.splice(document_gravel.indexOf(wName), 1);
    if (document_list.indexOf(wName) === -1) {
      document_list.push(wName);
    };
...
function syncImp(sid) {//url末尾に{?sid=*}があり同期を開始。それだけ(完成)
  if (localStorage.getItem("world_sync")) {
    ...
    window.location.href = 'https://aya-0p.github.io/sim/default/';//変更
  } else {
    localStorage.setItem("world_sync", sid);
    var t = JSON.parse(localStorage.getItem("document_list"))
    t.push("同期ワールド")
